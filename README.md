...
Сторонние компоненты в каталоге `js/vendor` распространяются по собственным лицензиям. Для HEIC/HEIF используются libheif, libde265 и Kvazaar, собранные в WebAssembly без x265. Исходный код связующего модуля, сведения о версиях и тексты лицензий находятся в `js/vendor/heic`. Условия этих лицензий относятся только к соответствующим библиотекам и не меняют условия использования AKD Image в целом.

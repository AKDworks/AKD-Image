...
AKD Image можно установить как PWA на компьютер или телефон со страницы [«Приложение»](https://image.akdworks.com/app). Установленная версия запускается в отдельном окне, автоматически получает обновления и сохраняет основные страницы и ресурсы для работы без интернета. Dropbox, другие облачные источники, первая загрузка отдельных ИИ- и видео-компонентов, а также обновления требуют подключения к сети. Пользовательские изображения и результаты в офлайн-кэш не добавляются.
...
- добавление и настройка текстового водяного знака;
- настройка яркости, контраста, насыщенности и других эффектов;
- пикселизация и размытие выбранной области;
- удаление фона с помощью локальной AI-модели и сохранение прозрачного PNG;
- создание мемов;
...
- оптимизация и изменение размера GIF-анимаций;
...
Единственная внешняя загрузка во время обработки – AI-модель, необходимая для удаления фона. Выбранное изображение остаётся на устройстве пользователя. Серверы, с которых загружаются модель и среда выполнения, могут получить обычные технические сведения о подключении, например IP-адрес и User-Agent.
...
Полноценная работа с анимированными GIF доступна в оптимизации GIF, изменении размера, водяном знаке, обрезке, обрезке по времени, повороте, фотоэффектах, генераторе мемов, скруглении углов, пикселизации, Base64, размытии области и пометках на изображении.
...
- автоматический выбор русского, английского или испанского языка по настройкам браузера;
- ручное переключение языка с локальным сохранением выбора;
...
- установка как PWA на компьютер или телефон;
- офлайн-доступ к основным локальным инструментам;
...
- до 75 МБ на одно статическое изображение;
- до 50 файлов и 500 МБ за одну загрузку;
...
Для удаления фона действуют отдельные ограничения:

- одно статичное изображение за одну обработку;
- до 40 МБ на исходный файл в пределах общих ограничений разрешения;
- при первом запуске загружается около 94 МБ для WebGPU или 183 МБ для запасного варианта WebAssembly;
- по возможности используется WebGPU, а скорость и расход памяти зависят от устройства.

Для анимированных GIF действуют отдельные ограничения:

- до 50 МБ на файл;
- до 300 кадров и 120 секунд;
- до 2560 пикселей по одной стороне и 4 мегапикселей на кадр;
- до 48 миллионов пикселей суммарно для всех кадров;
- не менее 30 мс на один кадр;
- до 100 МБ для готового GIF.

Для инструмента Base64 действует отдельный лимит – до 10 МБ исходных данных при преобразовании изображения в строку и обратно. Это ограничение защищает браузер от зависания при работе с очень длинными строками. Для более крупных изображений сначала используйте сжатие или изменение размера.

Для инструмента «Видео ↔ GIF» действуют отдельные ограничения:

- один файл за одну операцию;
- видео до 200 МБ, GIF до 50 МБ;
- до 120 секунд длительности;

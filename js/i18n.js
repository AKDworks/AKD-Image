...
(function () {
  ...
  const ENGLISH = {
    ...
    'Отделите главный объект от фона с помощью локальной AI-обработки и сохраните прозрачный результат.': 'Separate the main subject from the background with local AI processing and save it with transparency.',
    'Удалить фон – AKD Image': 'Remove background – AKD Image',
    'Удаляйте фон с изображения локально в браузере с помощью AI и скачивайте результат в PNG с прозрачностью.': 'Remove image backgrounds locally in your browser with AI and download a transparent PNG result.',
    'Отделите главный объект от фона с помощью локальной AI-обработки и скачайте PNG с прозрачностью.': 'Separate the main subject from the background with local AI processing and download a transparent PNG.',
    'Удаление фона': 'Background removal',
    'Предпросмотр исходного изображения': 'Source image preview',
    'При первом запуске браузер загрузит AI-модель размером около 94 МБ для WebGPU или 183 МБ для WASM. Изображение останется на вашем устройстве, а модель сохранится в кэше браузера.': 'On first use, the browser downloads an AI model of about 94 MB for WebGPU or 183 MB for WASM. Your image remains on your device, and the model is stored in the browser cache.',
    'Удаление фона...': 'Removing background...',
    'Проверка возможностей браузера...': 'Checking browser capabilities...',
    'Загрузка AI-модели...': 'Loading AI model...',
    ...
    '3. Библиотеки и AI-модель': '3. Libraries and AI model',
    ...
    'Фирменная технология локальной AI-обработки изображений от AKDworks.': 'AKDworks’ signature technology for local AI image processing.',
    '– Фирменная технология локальной AI-обработки изображений от AKDworks.': '– AKDworks’ signature technology for local AI image processing.',
    '– Собственная технология локальной AI-обработки изображений от AKDworks.': '– Proprietary technology for local AI image processing by AKDworks.',
    'Собственная технология локальной AI-обработки изображений от AKDworks.': 'Proprietary technology for local AI image processing by AKDworks.',
    'AKD Vision объединяет собственную библиотеку обработки, выполнение AI-моделей и оптимизированные сценарии для Web Worker, WebGPU и WebAssembly. Пользовательские изображения обрабатываются локально и не отправляются на сервер AKD Image.': 'AKD Vision combines a proprietary processing library, AI model execution and optimized workflows for Web Workers, WebGPU and WebAssembly. User images are processed locally and are not sent to the AKD Image server.',
    ...
    'Локальная AI-обработка изображений и удаление фона через Web Worker, WebGPU или WebAssembly.': 'Local AI image processing and background removal through a Web Worker, WebGPU or WebAssembly.',
    ...
    'Выполнение нейросетевой модели через WebGPU или WebAssembly непосредственно в браузере.': 'Runs the neural network model directly in the browser through WebGPU or WebAssembly.',
    ...
    'При удалении фона изображение также обрабатывается локально через AKD Vision. При первом запуске браузер загружает AI-модель и среду выполнения, сохраняет их в кэше и затем применяет модель к изображению на устройстве.': 'Background removal also processes the image locally through AKD Vision. On first use, the browser downloads the AI model and runtime, stores them in its cache and then applies the model to the image on your device.',
    ...
    '– локальная AI-обработка изображения и удаление фона;': '– provides local AI image processing and background removal;',
